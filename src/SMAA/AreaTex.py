...
def hook_format(image_data, name):
    ...
    with open(f'{name}.hook', 'w') as f:
        ...
        for y in range(height -1, -1, -1):
            for x in range(width):
                ...
                r_u8 = int(np.clip(val[0], 0, 1) * 255)
                g_u8 = int(np.clip(val[1], 0, 1) * 255)
                #b_u8 = 0
                #a_u8 = 255
...
def areaortho(pattern, left, right, offset):
    ...
    def area(p1, p2, x):
        ...
        if abs(d[0]) < 1e-9: return 0.0, 0.0
        ...
        if inside:
            ...
            if istrapezoid:
                ...
                # Check for division by zero
                if abs(d[1]) < 1e-9: return 0.0, 0.0
                x = -p1[1] * d[0] / d[1] + p1[0]
...
def assemble(tex4d_data, areas, edges, pos, size, compress):
    ...
    def putpattern(pattern):
        for left in range(size):
            for right in range(size):
                ...
                # Get value from source numpy array
                # Note: numpy is (y, x) indexed
                val = areas[pattern][p_comp[1], p_comp[0]]

                # Calculate destination coordinates
                pp = pos + p + vec2(size, size) * vec2(*edges[pattern])

                # Write to destination numpy array
                tex4d_data[pp[1], pp[0]] = val
...
def tex2dortho(args):
    ...
    tex2d_data = np.zeros((size, size, 2), dtype=np.float32)
    for y in range(size):
        for x in range(size):
...
def tex2ddiag(args):
    pattern, offset = args
    tex2d_data = np.zeros((SIZE_DIAG, SIZE_DIAG, 2), dtype=np.float32)
...
def tex4dortho(tex4d_data, y, offset):
    # Build each pattern subtexture concurrently:
    cores = max(1, cpu_count() - 1)
    with Pool(processes=cores) as pool:
        areas = pool.map(tex2dortho, [(i, offset) for i in range(16)])
    ...
    assemble(tex4d_data, areas, edgesortho, pos, SIZE_ORTHO, lambda v: (v[0]**2, v[1]**2))

# Calculate the diagonal patterns 4D texture for a given offset:
def tex4ddiag(tex4d_data, y, offset):
    # Build each pattern subtexture concurrently:
    cores = max(1, cpu_count() - 1)
    with Pool(processes=cores) as pool:
        areas = pool.map(tex2ddiag, [(i, offset) for i in range(16)])

    # Then, assemble the 4D texture:
    pos = vec2(5 * SIZE_ORTHO, 4 * SIZE_DIAG * y)
    assemble(tex4d_data, areas, edgesdiag, pos, SIZE_DIAG, lambda v: v);

#------------------------------------------------------------------------------
# Entry Point

if __name__ == '__main__':
    width = 2 * 5 * SIZE_ORTHO
    height = len(SUBSAMPLE_OFFSETS_ORTHO) * 5 * SIZE_ORTHO
    tex4d_data = np.zeros((height, width, 2), dtype=np.float32)

    for y, offset in enumerate(SUBSAMPLE_OFFSETS_ORTHO):
        tex4dortho(tex4d_data, y, offset)
    for y, offset in enumerate(SUBSAMPLE_OFFSETS_DIAG):
        tex4ddiag(tex4d_data, y, offset)

    preview_data_u8 = np.zeros((height, width, 4), dtype=np.uint8)
    preview_data_u8[..., 0:2] = (np.clip(tex4d_data, 0, 1) * 255).astype(np.uint8)
    preview_data_u8[..., 3] = 255

    img = Image.fromarray(preview_data_u8, 'RGBA')
    img.save("AreaTex.tga")

    hook_format(tex4d_data, "AREA_TEX")
